...
public class MultimodalEmbeddingsRequest {
  ...
  /**
   * 模型 ID 或 Endpoint ID（必选）
   * 需先开通模型服务并查询 Model ID，或使用 Endpoint ID 获得高级能力
   */
  @JsonProperty("model")
  ...
  /**
   * 输出向量维度（可选，默认值：2048）
   * 取值范围：1024 或 2048（仅 doubao-embedding-vision-250615 及后续版本支持）
   */
  @Builder.Default
  ...
  /**
   * 多向量开关配置（可选，默认值：type="disabled"）
   * type 取值：disabled（仅输出稠密向量）、enabled（同时输出单稠密向量和多向量列表）
   */
  @Builder.Default
  ...
  /**
   * 稀疏向量开关配置（可选，仅纯文本输入支持）
   * type 取值：disabled（仅输出稠密向量）、enabled（同时输出稠密向量和稀疏向量）
   */
  ...
  public static class Input {
    /**
     * 输入内容类型（必选）
     * 取值：text（文本）、image_url（图片）、video_url（视频）
     */
    @JsonProperty("type")
    private String type;

    /**
     * 文本内容（仅 type="text" 时必选）
     * 要求：utf-8 编码，长度≤100000 字节，单条 token 数≤8k，总 token 数建议≤4096 或条数≤4
     */
    @JsonProperty("text")
    private String text;
  ...
  public static class ImageUrl {
    /**
     * 图片地址（必选）
     * 支持：可访问的 URL 或 Base64 编码（格式：data:image/{格式};base64,{编码内容}）
     */
    @JsonProperty("url")
    private String url;
  }
  ...
  public static class VideoUrl {
    /**
     * 视频地址（必选）
     * 支持：可访问的 URL 或 Base64 编码，格式要求：.mp4、.avi、.mov（小写），文件≤50MB
     */
    @JsonProperty("url")
    private String url;
  }
