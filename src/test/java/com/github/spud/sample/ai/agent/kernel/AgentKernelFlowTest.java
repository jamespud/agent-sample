...
class AgentKernelFlowTest {
    ...
    void shouldCompleteWithFinalAction() {
        ...
        verify(stateMachineDriver).sendEvent(eq(stateMachine), eq(AgentEvent.THINK_DONE_NO_TOOLS), any());
    }
    ...
    void shouldExecuteToolAndReturnFinalAnswer() {
        ...
        // 验证工具执行仅在 ACT 阶段调用
        verify(toolExecutionService, times(1)).execute("retrieve_knowledge", "{\"query\":\"test query\"}");

        // 验证状态转移
        verify(stateMachineDriver).sendEvent(eq(stateMachine), eq(AgentEvent.START), any());
        verify(stateMachineDriver).sendEvent(eq(stateMachine), eq(AgentEvent.THINK_DONE_WITH_TOOLS), any());
